...
Web.Membership.prototype = {
    ...
    get_welcome: function () {
        return String.isNullOrEmpty(this._welcome) ? Web.MembershipResources.Bar.Welcome : this._welcome;
    },
    ...
    showPermalink: function () {
        if (!this._savePermalinkParams || String.isNullOrEmpty(this._savePermalinkParams.html)) {
            $appfactory.alert(Web.MembershipResources.Messages.PermalinkUnavailable);
            return;
        }
    ...
    _renderHistory: function () {
        ...
        for (var i = cardCount - 1; i >= 0; i--) {
            ...
            sb.append('</div>');
            if (i == 0)
                sb.appendFormat('<a id="{0}_RotateHistory" class="RotateHistory" href="javascript:" onclick="$find(\'{0}\')._rotateHistory(event);return false;"><div id="{0}_RotateButton" title="{3}" style="position:absolute;z-index:600002;left:{1}px;top:{2}px;" class="RotateHistory">&nbsp;</div></a>', this.get_id(), x + 110, y - 15, Web.MembershipResources.Bar.RotateHistory);
            x += dx;
            y += dy;
        }
        sb.append('</div>');
    ...
    _showHistory_Success: function (result) {
        if (result.length == 0) {
            $appfactory.alert(Web.MembershipResources.Messages.HistoryUnavailable)
            return;
        }
    ...
    idleInterval: function (enable) {
        ...
        if (enable)
            if (that.get_idleTimeout() > 0)
                that._idleIntervalId = window.setInterval(function () {
                    that.idle();
                }, 60000);
    },
    updated: function () {
        Web.Membership.callBaseMethod(this, 'updated');
        var barResources = Web.MembershipResources.Bar,
            that = this;
        that.idleInterval(true);
        that._updateLastActivity();
        if (!String.isNullOrEmpty(that.get_cultures())) {
            that._cultureList = [];
            var cultures = String.format('Detect,Detect|{0}|False;{1}', Web.MembershipResources.Bar.AutoDetectLanguageOption, that.get_cultures());
    ...
    _login_Completed: function (validCredentials) {
        ...
        if (!validCredentials) {
            var that = this;
            $appfactory.alert(Web.MembershipResources.Messages.InvalidUserNameAndPassword, function () {
            ...
            window.location.replace($appfactory.unanchor(returnUrl ? unescape(returnUrl[1]) : window.location.href));
        }
    },
    ...
    login: function () {
        ...
        if (this._userName && this._password) {
            ...
            if (isBlank.exec(this._userName.value)) {
                $appfactory.alert(Web.MembershipResources.Messages.BlankUserName, function () {
            ...
            if (isBlank.exec(this._password.value)) {
                ...
                $appfactory.alert(Web.MembershipResources.Messages.BlankPassword, function () {
                    that.set_isLoginDialogVisible(false);
