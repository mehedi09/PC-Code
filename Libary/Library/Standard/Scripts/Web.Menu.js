...
Web.Menu.prototype = {
    ...
    updated: function () {
        ...
        var nodes = this.get_nodes(),
            wdvrm = Web.DataViewResources.Menu;
        if (!nodes) return;
        ...
        if (this.get_showSiteActions()) {
            ...
            var itemIndex = Web.PageState.read(this.get_id());
            if (itemIndex != null) {
    ...
    _createMenu: function (nodes, parent, level) {
        ...
        for (i = 0; i < nodes.length; i++) {
            ...
            if (!String.isNullOrEmpty(url)) {
                ...
                if (url.match(Web.Menu.UrlRegex.match)) {
                    link.attr('menuItemUrl', url);
                    link.click(function (event) {
                        event.preventDefault();
                        Web.Menu.navigate(this.getAttribute('menuItemUrl'))
                    });
                }
    ...
    select: function (index, source) {
        ...
        if (!String.isNullOrEmpty(n.elementId)) {
            Web.PageState.write(this.get_id(), index);
...
Web.Menu.findNode = function (url, root) {
    ...
    for (var i = 0; i < root.length; i++) {
        ...
        if (n.url.match(url))
            return n;
        if (n.children) {
            var result = Web.Menu.findNode(url, n.children);
            if (result)
                return result;
        }
    }
...
Web.Menu.navigate = function (url) {
    ...
    var m = url.match(Web.Menu.UrlRegex);
    if (m)
        window.open(m[2], m[1]);
    else {
...
Web.HoverMonitor.prototype = {
    ...
    hover: function (elem, family, cssClass, depth) {
        ...
        if (isOpen) {
            if (that.get_hoverFrame().element != that.get_openFrame().element && openStyle == Web.HoverStyle.Auto || openStyle == Web.HoverStyle.ClickAndStay)
                that.hidePopups();
        }
        if (that._skipOpen) {
    ...
    startClose: function () {
        ...
        this._closeInterval = setInterval(function () {
            var hm = Web.HoverMonitor._instance;
            hm.stopClose();
            hm.close();
        }, 1000);
    },
    closeItem: function (family, path) {
        this.stopCloseItem();
        var item = family != null && path != null ? Web.Item.find(family, path) : this.get_activeItem();
    ...
    startCloseItem: function (family, path) {
        ...
        this._closeItemInterval = setInterval(function () {
            Web.HoverMonitor._instance.closeItem(family, path);
        }, 1000);
    },
    stopOpen: function () {
    ...
    startOpen: function (family, path) {
        ...
        if (!this._skipStartOpen) {
            this._openInterval = setInterval(function () {
                Web.HoverMonitor._instance.open(family, path);
            }, Web.HoverMonitor._instance._tempOpenDelay != null ? Web.HoverMonitor._instance._tempOpenDelay : 500);
            Web.HoverMonitor._instance._tempOpenDelay = null;
        }
...
function $closeHovers() {
    var instance = Web.HoverMonitor._instance;
    if (instance)
        instance.close()
}
...
if (__ie6)
    Sys.UI.DomElement.addCssClass(document.body, 'IE6');
